...
// Users can use this to catch Left/Right pad presses on options
int GuiOptionBrowser::GetIncDecOption(int dir)
{
	int found = -1;
	for(int i=0; i<PAGESIZE; i++)
	{
		if (dir == INCREMENT_VALUE)
		{
			if(optionBtn[i]->GetState() == STATE::INCREMENT)
			{
				optionBtn[i]->SetState(STATE::SELECTED);
				found = optionIndex[i];
				break;
		...
		if (dir == DECREMENT_VALUE)
		{
			if(optionBtn[i]->GetState() == STATE::DECREMENT)
			{
				optionBtn[i]->SetState(STATE::SELECTED);
				found = optionIndex[i];
				break;
...
void GuiOptionBrowser::Update(GuiTrigger * t)
{
	...
	// For Left/Right pad presses, expose STATE_{DE,IN}CREMENT 
	// to users if the useIncDec member is set to 1
	if (this->useIncDec == 1)
	{
		if(t->Right())
		{
			optionBtn[selectedItem]->SetState(STATE::INCREMENT);
		}
		else if(t->Left())
		{
			optionBtn[selectedItem]->SetState(STATE::DECREMENT);
		}
	}
