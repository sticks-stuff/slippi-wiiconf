...
int GuiOptionBrowser::GetIncDecOption(int dir)
{
	...
	for(int i=0; i<PAGESIZE; i++)
	{
		if (dir == INCREMENT)
		{
			if(optionBtn[i]->GetState() == STATE_INCREMENT)
			{
				optionBtn[i]->SetState(STATE_SELECTED);
				found = optionIndex[i];
				break;
		...
		if (dir == DECREMENT)
		{
			if(optionBtn[i]->GetState() == STATE_DECREMENT)
			{
				optionBtn[i]->SetState(STATE_SELECTED);
				found = optionIndex[i];
				break;
...
void GuiOptionBrowser::Update(GuiTrigger * t)
{
	...
	if (this->useIncDec == 1)
	{
		if(t->Right())
		{
			optionBtn[selectedItem]->SetState(STATE_INCREMENT);
		}
		else if(t->Left())
		{
			optionBtn[selectedItem]->SetState(STATE_DECREMENT);
		}
	}
